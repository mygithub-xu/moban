<template>
  <div class="edit-frame">
    <div class="edit-frame-left">
        ...
        <template v-if="needParam.isShowQuery">
          ...
          <div class="container-query">
            <template v-for="(item,index) in needParam.queryList">
              <!--普通查询框 -->
              <queryItem v-if="item.type == '1'"
                         :label="item.title+'：'"
                         :key="index">
                <el-input clearable></el-input>
              </queryItem>
              <!--下拉选择器 -->
              <queryItem v-if="item.type == '2'"
                         :label="item.title+'：'"
                         :key="index">
            ...
            <query-item isButton
              ...
              <template v-for="(item,index) in needParam.queryList">
                <el-button style="margin-left:10px"
                           v-if="item.type == '4'&&item.value == '4'"
                           :key="index"
                           @click="handleAdd">{{item.title}}</el-button>
                <el-button style="margin-left:10px"
                           v-if="item.type == '4'&&item.value != '4'"
            ...
            <div class="common-table-style">
              ...
              <el-table :data="pageData.list"
                ...
                <el-table-column type="selection"
                                 width="55"
                                 v-if="needParam.isShowCheckTable"></el-table-column>
                <el-table-column type="index"
                                 width="50"
                                 label="#"></el-table-column>
                <el-table-column v-for="(item,index) in needParam.tableList"
                                 :key="index"
                                 :prop="item.value"
                                 :label="item.title">
                  aaaaa
      ...
      <editTem ref="editTem"
               :editList="needParam.editList"></editTem>
      <!--上下结构 end-->
    </div>

    <div class="edit-frame-right"
         :style="{height:frameRight+'px'}">
      <el-scrollbar style="height:100%">
        <div class="line-div">
          <el-button class="line-div-button"
                     @click="back">返回</el-button>
          <el-button class="line-div-button"
                     @click="saveParm"
                     v-show="!isFromTem">保存</el-button>
          <el-button class="line-div-button"
                     @click="openDigCode"
                     v-show="!isFromTem">代码生成</el-button>
          <el-button class="line-div-button"
                     @click="openDigTem"
                     v-show="isFromTem">保存模板</el-button>
        ...
        <div class="line-div">
          <el-radio v-model="needParam.layoutType"
                    label="1">上查下表</el-radio>
          <el-radio v-model="needParam.layoutType"
                    label="2">左树右表</el-radio>
        </div>
        ...
        <el-collapse class="data-area"
                     v-model="activeName"
                     accordion>
          <el-collapse-item v-show="needParam.isShowQuery"
                            title="查询区域"
                            name="1">
            <div class="add-query">
              <div class="add-query-input">
                <el-select v-model="addType"
                           placeholder="请选择"
                           clearable>
                  <el-option v-for="item in addTypeList"
                             :key="item.value"
                             :label="item.label"
            ...
            <div class="data-area-item"
                 v-for="(item,index) in needParam.queryList"
                 :key="index">
              <i type="text"
                 class="data-area-button el-icon-delete"
                 @click="delItem(index)"></i>
              <template v-if="item.type != '4'">
                <el-input class="data-area-input"
                          placeholder="请输入查询标题"
                          v-model="item.title"></el-input>
                <el-select class="data-area-input"
                           v-model="item.value"
                           placeholder="关联字段"
                           clearable>
                  <el-option v-for="item in tableFileds"
                             :key="item.id"
                             :label="item.fieldName"
                             :value="item.id"></el-option>
                </el-select>
              ...
              <template v-if="item.type == '4'">
                ...
                <el-select class="data-area-input"
                           ...
                           placeholder="请选择"
                           clearable>
                  <el-option v-for="item in buttonDataSource"
                             :key="item.value"
                             :label="item.label"
                             :value="item.value"></el-option>
                </el-select>
          ...
          <el-collapse-item v-show="needParam.isShowTable"
                            title="列表区域"
                            name="2">
            <div class="add-query">
              <div class="add-query-button">
                ...
                <el-checkbox style="margin-left:10px"
                             v-model="needParam.isShowCheckTable">多选</el-checkbox>
            ...
            <div class="data-area-item"
              ...
              <el-select class="data-area-input"
                         ...
                         placeholder="请选择"
                         clearable>
                <el-option v-for="item2 in tableFileds"
                           :key="item2.id"
            ...
            <div v-show="needParam.isShowOperaTable">
              ...
              <div class="data-area-item"
                   v-for="(item,index) in needParam.tableButtonList"
                   :key="'operaTablekey-'+item.paramIndex">
                <i type="text"
                   class="data-area-button el-icon-delete"
                   @click="delOperaTableItem(index)"></i>
                <el-input class="data-area-input"
                          placeholder="请输入按钮名称"
                          v-model="item.title"></el-input>
                <el-select class="data-area-input"
                           ...
                           placeholder="请选择"
                           clearable>
                  <el-option v-for="item2 in tableButtonCheckList"
                             :key="item2.value"
                             :label="item2.label"
          ...
          <el-collapse-item v-show="needParam.isShowEdit"
                            title="编辑区域"
                            name="3">
            <div class="add-query">
              <div class="add-query-input">
                <el-select v-model="addEditType"
                           placeholder="请选择"
                           clearable>
                  <el-option v-for="item in addTypeList"
                             :key="item.value"
            ...
            <div class="data-area-item"
                 v-for="(item,index) in needParam.editList"
                 :key="index">
              <i type="text"
                 class="data-area-button el-icon-delete"
                 @click="delEditItem(index)"></i>
              <template v-if="item.type != '4'">
                <el-input class="data-area-input"
                          placeholder="请输入查询标题"
                          v-model="item.title"></el-input>
              ...
              <template v-if="item.type == '4'">
                ...
                <el-select class="data-area-input"
                           ...
                           placeholder="请选择"
                           clearable>
                  <el-option v-for="item in editButtonDataSource"
                             :key="item.value"
                             :label="item.label"
                             :value="item.value"></el-option>
                </el-select>
    ...
    <el-dialog title="提示"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="generateTemplate">确 定</el-button>
      </span>
    </el-dialog>
  </div>
