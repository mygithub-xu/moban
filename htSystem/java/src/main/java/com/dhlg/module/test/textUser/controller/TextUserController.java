package com.dhlg.module.test.textUser.controller;
 ...
 */
@RestController
@RequestMapping("/api/test/textUser")
@CrossOrigin
public class TextUserController {

        @Autowired
        ITextUserService doService;


        @ApiOperation("保存或者更新")
        @PostMapping("/saveOrUpdate")
        public Result saveOrUpdate(@RequestBody TextUser textUser) {
                if(StringUtils.isBlank(textUser)){
                        throw new ParamIsNullException();
                }
                return  doService.customSaveOrUpdate(textUser);
        }
